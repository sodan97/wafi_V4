...
const EditProductModal: React.FC<EditProductModalProps> = ({ product, onClose }) => {
  ...
  const handleAddImageUrl = () => {
    if (imageUrl.trim() && imageUrl.startsWith('http')) {
      setExistingImages(prev => [...prev, imageUrl.trim()]);
      setImageUrl('');
      setError('');
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const allImageUrls = [...existingImages, ...storageUrls];
        ...
        <div className="p-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-3 bg-gray-50 p-4 rounded-lg border">
                    ...
                    <div>
                        <label htmlFor="file-upload-edit" className="w-full cursor-pointer bg-white border-2 border-dashed border-gray-300 rounded-md p-4 flex flex-col items-center justify-center text-sm text-gray-600 hover:border-rose-400 hover:bg-rose-50 transition-colors">
                        ...
                        <input id="file-upload-edit" name="file-upload-edit" type="file" className="sr-only" multiple accept="image/png, image/jpeg, image/webp" onChange={handleFileChange} />
                    </div>
                            ...
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                                {existingImages.map((imgSrc, index) => (
                                    <div key={index} className="relative group aspect-square">
                                        <img src={imgSrc} alt={`Image ${index+1}`} className="w-full h-full object-cover rounded-md border" />
                                        <button type="button" onClick={() => handleRemoveExisting(index)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Supprimer">
                    ...
                    {imagePreviews.length > 0 && (
                        <div>
                            <p className="text-xs text-gray-500 mb-1">Nouveaux fichiers à uploader dans Storage :</p>
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                ...
                <div className="space-y-3 bg-blue-50 p-4 rounded-lg border border-blue-200">
                    ...
                    <div>
                        <label htmlFor="video-upload-edit" className="w-full cursor-pointer bg-white border-2 border-dashed border-blue-300 rounded-md p-4 flex flex-col items-center justify-center text-sm text-gray-600 hover:border-blue-400 hover:bg-blue-50 transition-colors">
                        ...
                        <input id="video-upload-edit" type="file" className="sr-only" accept="video/mp4,video/webm,video/ogg,video/quicktime" onChange={handleVideoFileChange} />
                    </div>
                    {!videoFile && (
                        <>
                            <div className="relative flex items-center">
                ...
                <div className="flex justify-end gap-4 pt-4">
                    ...
                    <button type="submit" disabled={isSubmitting} className="px-6 py-2 rounded-md font-semibold text-white bg-rose-500 hover:bg-rose-600 transition-colors disabled:opacity-60">
                        {isSubmitting ? 'Upload en cours...' : 'Enregistrer les modifications'}
                    </button>
                </div>
            </form>
        </div>
